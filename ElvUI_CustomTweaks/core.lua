...
-- Built lazily (not at file scope) so the tweak descriptions resolve against the
-- player's chosen language, which is only known after login. See ElvUI Core.lua.
local function GetTweaks() return {
	["Actionbar"] = {
...
function CT:ConfigTable()
	...
	for cat, tweaks in pairs(GetTweaks()) do
